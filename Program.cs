...
using ISOFile = ISOParser.ISOFile;
...
namespace KH1FM_Toolkit
{
    ...
    public class KH1ISOReader : IDisposable
    {
        ...
        /// <summary>
        ///     <para>Sorted list of IDX entries</para>
        ///     <para>Sorted by LBA</para>
        /// </summary>
        ...
        /// <summary>
        ///     <para>Offset, in bytes, of all data.</para>
        ///     <para>This value is added to the IDX offset to get a file's offset.</para>
        /// </summary>
    ...
    internal class KH1ISOWriter : IDisposable
    {
        ...
        private long headerIMGEnd;
        private long headerIMGoffset;

        /// <summary>
        ///     <para>Unsorted list of IDX entries</para>
        ///     <para>Sorted in <c>finalize</c> by Hash</para>
        /// </summary>
        /// <remarks>
        ///     <para>Default size of 16 is because there are normally 16 items in the ISO up to, and including, the IDX.</para>
        ///     <para>Once at the IDX, a more realistic Capacity is set.</para>
        /// </remarks>
        ...
        /// <summary>
        ///     <para>Original ISO.</para>
        ///     <para>Used for copying data from</para>
        /// </summary>
        private KH1ISOReader origISO;

        private bool updateISOHeaders;
        ...
        private static UInt64 _ISO9660Number(UInt32 i)
        {
            return i | (UInt64) (i & 0x000000FFU) << 56 | (UInt64) (i & 0x0000FF00U) << 40 |
                   (UInt64) (i & 0x00FF0000U) << 24 | (UInt64) (i & 0xFF000000U) << 8;
        }
        ...
        private void copyBytes(long length, long srcOffset = -1, long dstOffset = -1)
        {
            ...
            if (srcOffset >= 0)
            {
                origISO.iso.Position = srcOffset;
            }
            if (dstOffset >= 0)
            {
                iso.Position = dstOffset;
            }
            int bytesRead;
            while (length > 0 &&
                   (bytesRead =
                       origISO.iso.Read(copyBuffer, 0, copyBuffer.Length < length ? copyBuffer.Length : (int) length)) >
                   0)
            {
        ...
        /// <summary>
        ///     <para>Update file information in the ISO 9660 filesystem</para>
        ///     <para>Also tracks IMG end offset</para>
        /// </summary>
        ...
        public bool writeHeader(string exthead = "")
        {
            if (iso.Length > 0)
            {
                throw new NotSupportedException("Can't write header when there's already datas!");
            }
        ...
        public void writeDummyIDX(int entryC)
        {
            if (dataOffset == 0)
            {
                throw new NotSupportedException("No header written");
            }
            if (idxOffset != 0)
            {
                throw new NotSupportedException("IDX already written");
            }
            idxOffset = iso.Position;
            idxSize = (long) Math.Ceiling((decimal) entryC*16/2048)*2048; //Round up to nearest 2048
            iso.Position += idxSize - 1;
            iso.WriteByte(0);
        ...
        public void writeBytes(byte[] data, UInt32 hash, UInt32 flags)
        {
            ...
            {
                throw new Exception("File too large to write");
            }
        ...
        public void importFile(Stream str, UInt32 hash, UInt32 flags = 0)
        {
            ...
            int bytesRead;
            while (length > 0 &&
                   (bytesRead = str.Read(copyBuffer, 0, copyBuffer.Length < length ? copyBuffer.Length : (int) length)) >
                   0)
            {
                iso.Write(copyBuffer, 0, bytesRead);
                length -= bytesRead;
    ...
    internal static class NativeMethods
    {
        ...
        /// <remarks>
        ///     .NET 4 and higher have this built-in to the <c>FileStream.Flush</c> function, but some early implementations
        ///     of it are known to be buggy.
        /// </remarks>
